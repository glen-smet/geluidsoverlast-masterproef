...
//	PRINTF_LN("tickstart: %d",tickstart);
//	PRINTF_LN("tickNow: %d",tickNow);
//	PRINTF_LN("timeout: %d",timeout);
	while(!totalReplyBufferDone && ( ( tickNow - tickstart )  < timeout)){
//		PRINTF("%d, ",( tickNow - tickstart ));
    SCH_Run(); 
		//HAL_Delay(10);
...
BG96_Status_t BG96_PowerOn( void ){
  ...
  HAL_GPIO_WritePin(BG96_RESETKEY_PORT, BG96_RESETKEY_PIN, GPIO_PIN_RESET); 
	...
	#endif
	PRINTF_LN("WAIT FOR POWER");
//	PRINTF_LN("uart state: %d",HAL_UART_GetState(&BG96_UARTHandle));
//	PRINTF_LN("uart error: %d",HAL_UART_GetError(&BG96_UARTHandle));
...
BG96_Status_t BG96_UDP_SendData( char * data, uint32_t timeout){
	char buffer[60];	
	BG96_Status_t status = BG96_ERROR;
	sprintf(buffer, "AT+QISEND=%d,1449\r\n", _connectID);
	uint32_t tickstart = HW_RTC_GetTimerValue();
	uint32_t tickNow = HW_RTC_GetTimerValue();
	PRINTF_LN("send 1");	
//	status = BG96_SendATCommandCheckReply(buffer, ">", 500);
	while(status != BG96_OK && ( ( tickNow - tickstart ) ) < timeout){
		HAL_Delay(500+rand()%500);
		status = BG96_SendATCommandCheckReply(buffer, ">", 2000);
		tickNow = HW_RTC_GetTimerValue();
	}
	PRINTF_LN("send 2");
	PRINTF_LN("send 3");
	...
	HAL_Delay(200);
	status = BG96_SendATCommandCheckReply("", " ", 3000);
	status = BG96_SendATCommandCheckReply("", "SEND OK", 2000);
	char buffer2[60];
	char bufferrx[20];
	PRINTF_LN("send data send OK");
	sprintf(buffer2, "AT+QISEND=%d,0\r\n", _connectID);
	PRINTF_LN("send 4");
	status = BG96_ERROR;
	...
	while(status != BG96_OK && ( ( tickNow - tickstart ) ) < timeout){
		HAL_Delay(1000+rand()%500);
		BG96_SendATCommandGetReply(buffer2, bufferrx, 10000);
		PRINTF_LN("response %s",bufferrx);
		for(int n=0;n<19;n++)
		{
			if(bufferrx[n] == 'O' && bufferrx[n] == 'K'){
				status = BG96_OK;
				PRINTF_LN("correct %s",bufferrx);}
		}
		if(status != BG96_OK){
			status = BG96_SendATCommandCheckReply("", "OK", 3000);	
		}
		tickNow = HW_RTC_GetTimerValue();
	}	
	return status;
